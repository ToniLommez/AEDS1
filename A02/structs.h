...
int_Array fIO_readBitArray(chars fileName)
{
    // definir dados locais
    int x = 0;
    int y = 0;
        ...
        else
        {
            ...
            if (array.data)
            {
                ...
                while (!feof(arquivo) && array.ix < array.length)
                {
                    ...
                    {
                        array.length--;
                        array.data = (ints) realloc (array.data, (array.length)*sizeof(int) );
                    }
                } // fim repetir
            }     // fim se
        }         // fim se
...
int decrescenteTransformar(int_Array array1){
    int iteracoes = 0;
    if (array1.length <= 0)
    {
    ...
    else
    {
        ...
        if (array1.data == NULL)
        {
            array1.length = 0; // nao ha' espaco
        }
        else
        {
            int temp = 0;
            bool continuar = true;
            for (array1.ix = 0; continuar && array1.ix < array1.length; array1.ix++){
                for(int j = array1.ix+1, continuar = false; j < array1.length; j++){
                    if (array1.data[array1.ix] < array1.data[j]){
                        temp = array1.data[array1.ix];
                        array1.data[array1.ix] = array1.data[j];
                        array1.data[j] = temp;
                        continuar = true;
                        iteracoes++;
                    }
...
int inverterArray(int_Array array1){
    int iteracoes = 0;
    if (array1.length <= 0)
    {
        printf("\nTamanho incompativel!!!\n\n");
...
int menorDeTodos(int_Array array1){
    int menor = 0;
    if (array1.length <= 0)
    {
        printf("\nTamanho incompativel!!!\n\n");
    ...
    else
    {
        ...
        else
        {
            ...
            for (int i = 0; i < (array1.length-1); i++){
                if (menor > array1.data[i+1])
                {
                    menor = array1.data[i+1];
                }
            }
        }// fim se
    }// fim se
...
int segundoMenorDeTodos(int_Array array1, int menor){
    int segundoMenor = -2147483647;
    if (array1.length <= 0)
    {
        printf("\nTamanho incompativel!!!\n\n");
    }
    else
    {
        ...
        {
            for (int i = 0; i < array1.length && segundoMenor <= menor; i++) // evitar que 'SEGUNDOMENOR' seja menor do que 'MENOR'
            {
                if (segundoMenor <= menor)
                {
                    segundoMenor = array1.data[i];
                }
            }
            for (int i = 0; i < (array1.length-1); i++){
                if (segundoMenor > array1.data[i+1] && array1.data[i+1] > menor)
                {
                    segundoMenor = array1.data[i+1];
                }
            }
        }// fim se
    }// fim se
...
int_Array valoresComuns(int_Array array1, int_Array array2){
    if (array1.length != array2.length)
    {
        printf("Tamanhos incompativeis, retornando array1");
        return array1;
    ...
    else
    {
        ...
        else
        {
            ...
            {
                j = 0;
                if (isPresent(array1.data[i], filtrados))
                {
                    j = array2.length;
                }
                for (; j < array2.length; j++)
                {
                    ...
                    {
                        filtrados.length++;
                        filtrados.data = (ints) realloc (filtrados.data, (filtrados.length)*sizeof(int) );
                        filtrados.ix++;
                        filtrados.data[filtrados.ix] = array1.data[i];
                        j = array2.length;
                    }
                }
            }
...
int paraDecimal(int_Array array1){
    int decimal = 0;
    if (array1.length <= 0)
    {
        printf("\nTamanho incompativel!!!\n\n");
    ...
    else
    {
        ...
        {
            for (int posicao = (array1.length-1), potencia = 0; posicao >= 0; posicao--, potencia++)
            {
                decimal += array1.data[posicao] * ((int)pow(2, potencia));
            }
        }// fim se
    }// fim se
    return decimal;
...
ref_int_Matrix tridiagonalCrescente(int tamanho)
{
    ref_int_Matrix tmpMatrix;
    tmpMatrix = new_int_Matrix(tamanho, tamanho);
    if (tamanho > 0)
    {
        int n = 0;
        for(int i = 0; i<tmpMatrix->rows; i++){
            for(int j = 0; j<tmpMatrix->columns; j++){
                if(i - 1 <= j && j <= i + 1){
                    n++;
                    tmpMatrix->data[i][j]=n;
                }else
                    tmpMatrix->data[i][j]=0;
            }
        }
    }
    return (tmpMatrix);
...
ref_int_Matrix tridiagonalDecrescente(int tamanho)
{
    ref_int_Matrix tmpMatrix;
    tmpMatrix = new_int_Matrix(tamanho, tamanho);
    if (tamanho > 0)
    {
        ...
        for(int i = 0; i<tmpMatrix->rows; i++){
            for(int j = 0; j<tmpMatrix->columns; j++){
                if(i - 1 <= j && j <= i + 1){
                    n--;
                    tmpMatrix->data[j][i]=n;
                }else
                    tmpMatrix->data[j][i]=0;
            }
        }
    }
    return (tmpMatrix);
...
bool potenciaPorLinhas(ref_int_Matrix matrix){
    bool correto = true;
    if (matrix->rows <= 0 || matrix->columns <= 0)
    {
        printf("matriz invalida");
    ...
    else if (matrix->data)
    {
        for (matrix->ix = 0; matrix->ix < matrix->rows; matrix->ix = matrix->ix + 1)
        {
            for (matrix->iy = 0; matrix->iy < matrix->columns; matrix->iy = matrix->iy + 1)
            {
                if ((int)pow((matrix->ix + 1), matrix->iy) != matrix->data[matrix->ix][matrix->iy])
                {
                    correto = false;
                }
            } // fim repetir
...
bool potenciaPorColunas(ref_int_Matrix matrix){
    bool correto = true;
    if (matrix->rows <= 0 || matrix->columns <= 0)
    {
        printf("matriz invalida");
    ...
    else if (matrix->data)
    {
        for (matrix->ix = 0; matrix->ix < matrix->rows; matrix->ix = matrix->ix + 1)
        {
            for (matrix->iy = 0; matrix->iy < matrix->columns; matrix->iy = matrix->iy + 1)
            {
                if ((int)pow((matrix->ix + 1), matrix->iy) != matrix->data[matrix->iy][matrix->ix])
                {
                    correto = false;
                }
            } // fim repetir
